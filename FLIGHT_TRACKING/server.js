...
// Flight search route
app.get('/searchFlights/:departureCity,:destinationCity', async (request, response) => {
  try {
    // Retrieve departure and destination cities from the request body
    const { departureCity, destinationCity } = request.body;
    console.log(request.params.departureCity, request.params.destinationCity)

    // Fetch nearby airports for departure city
    const departureAirportsResponse = await fetch(`https://airlabs.co/api/v9/airports?city=${departureCity}&api_key=${myFlightsAPIKey}`);
    const departureAirportsData = await departureAirportsResponse.json();
    const departureAirports = departureAirportsData.response.dep_airports.map(airport => airport.iata_code);

    // Fetch nearby airports for destination city
    const destinationAirportsResponse = await fetch(`https://airlabs.co/api/v9/airports?city=${destinationCity}&api_key=${myFlightsAPIKey}`);
    const destinationAirportsData = await destinationAirportsResponse.json();
    const destinationAirports = destinationAirportsData.response.arr_airports.map(airport => airport.iata_code);

    // Fetch all flights from departure city to destination city
    const allFlightsResponse = await fetch(`https://airlabs.co/api/v9/flight?dep_iata=${departureAirports.join(',')}&api_key=${myFlightsAPIKey}`);
    const allFlightsData = await allFlightsResponse.json();
    const flightsToDestinationCity = allFlightsData.response.flights.filter(flight => destinationAirports.includes(flight.arr_iata));
    console.log('Search Flights Error:', flightsToDestinationCity)
    // Send the flight details back to the client
    response.json(flightsToDestinationCity);
  } catch (error) {
